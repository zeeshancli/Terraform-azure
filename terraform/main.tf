# Remote state backend (configured per environment via -backend-config files, e.g. envs/dev.backend.hcl)
terraform {
  backend "azurerm" {}
}

# Optionally create the Resource Group if one isn't supplied (see locals.create_rg and locals.rg_name)
...
# Networking (VNet, subnets, optional service endpoints)
module "network" {
...
# Private DNS zones (for Private Endpoints) and VNet link
module "dns" {
  source = "./modules/dns"
...
# Web App (with optional Private Endpoint or Service Endpoint mode)
module "web_app" {
  ...
  # Ensure DNS zone + VNet link exist before PE resolves
  depends_on = [module.dns]
}

# Azure SQL (Private Endpoint or Service Endpoint mode)
module "sql" {
...
# Application Gateway (routes to Web App default hostname)
module "app_gateway" {
  ...
  # Ensure DNS link exists so backend FQDN resolves to private IP when PE mode is used
